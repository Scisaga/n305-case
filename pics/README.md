...
- 01 图约 `100.0 mm`、02 图约 `105.5 mm` 的卡尺读数确认 PCB 平面尺寸；卡爪位于角部，不能据此得到接口最大包络，也不能把差值对称分配给 04/06。
- 当前用户粗测为：04 双 USB 是该面最突出接口，约超出 PCB `2.0 mm`；06 两个 USB 约超出 `1.0 mm`；04 鳍片不超出 PCB。其余 04 接口只确认“略凸”，逐件突出量仍为 provisional。
...
6. 卡尺存在遮挡和透视误差；01、02 用于 PCB 平面尺寸，不能当成接口鼻端总包络。不同 Z 高度的接口和鳍片还会产生不同俯拍视差，不能直接用 PCB 平面单应结果反算突出量。08～11 用于局部尺寸和交叉验证。
...
静态照片不能直接测得真实触发行程。原机 06 面照片和按钮照片支持恢复圆形按钮：可见帽约 Ø10.0 mm、止挡法兰约 Ø12.5 mm；用户实测穿过面板孔的按钮段为 **Ø9.2 mm**，原壳/新壳采用 **Ø9.4 mm** 配合孔。侧视照片中后端推杆被便签遮挡，不能从该图直接量取推杆长度。

[03 底面俯视照片](./03-n305-mainboard-bottom-side-overview.jpg) 与 [06 侧视照片](./06-n305-mainboard-top-dual-usb-edge-height-measurement.jpg) 共同确认：两个 USB 金属口构成06面的外侧接口基准；开关触点比 USB 前沿约向内退 `1.2 mm`（照片推定，暂记 ±0.4 mm），相邻 PCB 边还有局部台阶内缩。因此不能用 `100 mm` PCB 外接矩形的 `X=50` 直边直接推出开关触点位置，也不能把开关触点当作必须伸入 Ø9.4 墙孔的对象。按当前 USB 突出 `1.0 mm` 代理，开关触点暂置于 `X≈49.8 mm`；结合原壳 04–06 内距约 `103.0 mm`，06内墙暂置于 `X=51.0 mm`，两者约有 `1.2 mm` 静止轴向空间，后续由原按钮推杆跨越。

该标定取代早期“开关位于 05 边、距照片左边约 32 mm”的错误假设。

两只 USB-A 中心分别为 `Y≈+4.75 mm` 与 `Y≈-15.25 mm`。它们是同层安装的相同器件，因此机械参考共享 `Z=-3.14 mm`；此前 `-3.19/-3.09 mm` 的差值来自斜拍照片的透视推算，已经撤销。两个面板孔均采用用户确认的 **12.8 × 5.5 mm、R0.7** 水平圆角矩形；旧版 13.3/13.4 × 6.0 mm 以及 15.4 × 7.8 mm 孔作废。
...
装板时先把 06 边 USB 斜向送入上壳窗口，再放平主板和安装下壳。四个 PCB 安装孔轴同时连接主板及两壳，不设置额外壳体螺丝位。04、06 各自保留一整面带贯通开孔的连续实体墙，不承担贴边定位；原壳实测 04–06 内距约 `103.0 mm`，结合 04/06 USB 最大突出量暂分配为 `2.0/1.0 mm`。05/07 仍各留 `0.20 mm` 紧凑试装余量；原壳约 `110.5 mm` 的 05–07 内距包含无需复制的额外留空。侧壁厚采用原壳实测 `1.8 mm`。连续内搭边和连续卡槽已经从方案中取消，六段异件边采用平面对接；详细结构见 [../docs/v0.2-reference-review.md](../docs/v0.2-reference-review.md)。
